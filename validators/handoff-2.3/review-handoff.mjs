...
const dir = resolve(args.handoff); const specPreview = readJSON(`${dir}/functional-spec.json`); if (specPreview.schemaVersion !== '2.3') throw new Error('only implementation-handoff schema 2.3 is supported'); const manifestPreview = readJSON(`${dir}/handoff-manifest.json`); const inputMode = manifestPreview.inputMode; if (!['release-backed', 'design-led'].includes(inputMode)) throw new Error('handoff inputMode is invalid'); const semanticFiles = ['frontend-semantic-inventory.json', 'observed-interactions.json', 'control-capability-map.json', 'asset-role-inventory.json']; const sourceFiles = inputMode === 'release-backed' ? ['visual-source.json', 'release-manifest.json', 'suite-gate.json', 'visual-approval.json'] : ['design-source.json']; const files = ['handoff-manifest.json', ...sourceFiles, 'frontend-manifest.json', 'functional-spec.json', ...semanticFiles, 'handoff-anchor-manifest.json', 'visual-controls.json', 'ui-implementation-plan.json', 'api-contract.json', 'domain-bindings.json', 'runtime-contract.json'];
for (const file of files) if (!existsSync(`${dir}/${file}`)) throw new Error(`handoff is missing ${file}`);
const manifest = manifestPreview; const visual = inputMode === 'release-backed' ? readJSON(`${dir}/visual-source.json`) : null; const designSource = inputMode === 'design-led' ? readJSON(`${dir}/design-source.json`) : null; const release = inputMode === 'release-backed' ? readJSON(`${dir}/release-manifest.json`) : null; const frontend = readJSON(`${dir}/frontend-manifest.json`); const spec = readJSON(`${dir}/functional-spec.json`); const plan = readJSON(`${dir}/ui-implementation-plan.json`); const api = readJSON(`${dir}/api-contract.json`); const gate = inputMode === 'release-backed' ? readJSON(`${dir}/suite-gate.json`) : null; const approval = inputMode === 'release-backed' ? readJSON(`${dir}/visual-approval.json`) : null; const bindings = readJSON(`${dir}/domain-bindings.json`);
const errors = [];
...
} else if (designSource.sourceType !== 'finalized-design' || !designSource.designManifestDigest || manifest.designManifestDigest !== designSource.designManifestDigest || spec.visualSource?.designManifestDigest !== designSource.designManifestDigest) errors.push('handoff finalized design digest mismatch');
...
if (inputMode === 'release-backed' && (semanticInventory.release?.releaseDigest !== release.releaseDigest || interactions.releaseDigest !== release.releaseDigest || controlMap.releaseDigest !== release.releaseDigest || assetInventory.releaseDigest !== release.releaseDigest)) errors.push('handoff frontend semantics are not bound to the visual release');
if (inputMode === 'design-led' && (semanticInventory.design?.manifestDigest !== designSource.designManifestDigest || interactions.designManifestDigest !== designSource.designManifestDigest || controlMap.designManifestDigest !== designSource.designManifestDigest || assetInventory.designManifestDigest !== designSource.designManifestDigest)) errors.push('handoff presentation semantics are not bound to the finalized design');
if ((assetInventory.assets || []).some((item) => !['decorative', 'business-sample'].includes(item.role) || !item.digest || (item.role === 'business-sample' && !item.requiredReplacement))) errors.push('handoff asset roles are unclassified');
if (inputMode === 'release-backed') { const copiedSourceDigest = hashDirectory(`${dir}/web`); if (frontend.sourceTreeDigest !== copiedSourceDigest || visual.sourceTreeDigest !== copiedSourceDigest) errors.push('handoff web source tree digest mismatch'); }
...
for (const capabilityId of capabilityIds) {
  ...
  if (presentation?.mode !== 'headless' && presentation?.targetPageId && !Object.hasOwn(frontend.pages || {}, presentation.targetPageId)) errors.push(`capability ${capabilityId} target page is absent from the presentation source: ${presentation.targetPageId}`);
...
const receipt = { schemaVersion: '1.4', contractVersion: `implementation-handoff/${spec.schemaVersion}`, trustedReviewerId: `fdd-handoff-reviewer-${spec.schemaVersion}`, validatorDigest: treeDigest(resolve(trustedReviewerPath, '..')), inputMode, status: 'approved', authorAgentId: manifest.authorAgentId, reviewerAgentId: args['reviewer-agent'], functionalPackageDigest: manifest.functionalPackageDigest, ...(inputMode === 'release-backed' ? { visualReleaseDigest: manifest.visualReleaseDigest } : { designManifestDigest: manifest.designManifestDigest }), reviewedAt: new Date().toISOString() };
...
const protectedFiles = [...files, 'handoff-review-receipt.json']; const digests = Object.fromEntries(protectedFiles.map((file) => [file, sha(readFileSync(`${dir}/${file}`))])); if (inputMode === 'release-backed') digests.web = hashDirectory(`${dir}/web`);
writeJSON(`${dir}/handoff-lock.json`, { schemaVersion: '1.0', algorithm: 'sha256', inputMode, functionalPackageDigest: manifest.functionalPackageDigest, ...(inputMode === 'release-backed' ? { visualReleaseDigest: manifest.visualReleaseDigest, sourceTreeDigest: digests.web } : { designManifestDigest: manifest.designManifestDigest }), digests });
