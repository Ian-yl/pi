...
if (!existsSync(`${candidate}/campaign-contract.json`)) throw new Error('generic validation campaign requires candidate campaign-contract.json');
const candidateContract = readJSON(`${candidate}/campaign-contract.json`);
if (level === 'integrated' && !candidateContract.runtime?.integratedE2e) throw new Error('integrated validation requires runtime.integratedE2e in campaign-contract.json');
