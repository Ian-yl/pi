...
const designLed = process.argv.includes('--design-led');
...
let removedDesignCapability = null;

rmSync(output, { recursive: true, force: true });
mkdirSync(output, { recursive: true });
  run(`${fddRoot}/scripts/scaffold-package.mjs`, ['--input', architecture, ...(!designLed ? ['--visual-release', release] : []), '--output', functional, '--author-agent', 'golden-domain-author', '--designs', `${fixtures}/designs`]);
  overlayAuthoredClosure(functional);
  if (designLed) { authorDesignLedPresentationFixture(functional); removePlannedFixtureCapability(functional); }
...
run(`${fddRoot}/scripts/build-implementation-handoff.mjs`, ['--functional', functional, ...(!designLed ? ['--visual-release', release] : []), '--output', handoff, '--author-agent', 'golden-handoff-author']);
...
const planned = capabilities.find((item) => item.specificationStatus === 'planned') || removedDesignCapability;
if (!headlessOnly && (!submit || !upload || (!designLed && !planned))) throw new Error('neutral schema 2.3 authored fixture lacks its required aggregate, upload, or planned capabilities');
...
function writeFrontendImplementation() {
  ...
  if (designLed) { mkdirSync(`${implementation}/web/pages/submission/assets`, { recursive: true }); cpSync(`${fixtures}/designs/submission.svg`, `${implementation}/web/pages/submission/assets/icon.svg`); }
  ...
  let implementedHtml = uploadMultiple ? html.replace('type="file"', 'type="file" multiple').replace(`body.append(${JSON.stringify(uploadOperation.resourceTransfer.fileField)},event.target.files[0])`, `for(const file of event.target.files)body.append(${JSON.stringify(uploadOperation.resourceTransfer.fileField)},file)`) : html;
  if (designLed) {
    implementedHtml = implementedHtml.replace(`<button id="${controls.planned}" data-vr-id="${vr.planned}">${planned.name}</button>`, '');
    const handlerStart = implementedHtml.lastIndexOf(`document.querySelector('#${controls.planned}').addEventListener`); const scriptEnd = implementedHtml.lastIndexOf('</script>');
    if (handlerStart >= 0) implementedHtml = `${implementedHtml.slice(0, handlerStart)}${implementedHtml.slice(scriptEnd)}`;
  }
...
function writeBrowserTest(data) {
  ...
  let browserScript = script;
  if (designLed) { const start = browserScript.lastIndexOf(`await page.goto(process.env.BASE_URL+'/submission');const planned=`); const end = browserScript.lastIndexOf(`const traceArtifact=`); if (start >= 0) browserScript = `${browserScript.slice(0, start)}${browserScript.slice(end)}`; }
  writeFileSync(`${implementation}/tests/browser-runtime.mjs`, browserScript);
}
...
function writeFrontendContracts() {
  ...
  const bindings = [
    ...
    ...(!designLed ? [{ id: `binding-${planned.id}`, capabilityId: planned.id, bindingType: 'activation', mode: planned.presentation.mode, runtimeEvidenceId: 'browser-planned', source: { path: 'web/pages/submission/index.html', locator: `control-${planned.id}` } }] : [])
  ];
  ...
  writeJSON(`${implementation}/interaction-manifest.json`, { schemaVersion: '1.0', status: 'implemented', interactions: [
    ...
    ...(!designLed ? [{ id: `interaction-${planned.id}`, capabilityId: planned.id, event: 'click', evidenceId: 'browser-planned', status: 'planned' }] : [])
  ] });
...
// Test-only stand-in for the FDD Agent's authored design interpretation. Production scaffold code emits an
// empty design-led authoring workspace and never runs this fixture helper.
function authorDesignLedPresentationFixture(dir) {
  const designManifest = readJSON(`${dir}/design-manifest.json`);
  const designManifestDigest = sha(JSON.stringify(designManifest));
  const inventory = readJSON(`${dir}/frontend-semantic-inventory.json`);
  const controls = [['title-input', 'input', 'identity-section'], ['category-select', 'select', 'options-section'], ['quantity-input', 'input', 'options-section'], ['upload-input', 'file', 'upload-panel'], ['submit-button', 'button', 'workspace'], ['planned-export', 'button', 'workspace']].map(([controlId, kind, regionId]) => ({ controlId, referenceId: controlId, pageId: 'submission', kind, label: controlId, defaultValue: controlId === 'quantity-input' ? '2' : null, region: { id: regionId }, evidence: { status: 'designed', sources: ['design:submission'] } }));
  inventory.pages = [{ pageId: 'submission', route: '/submission', regions: ['workspace', 'identity-section', 'options-section', 'upload-panel', 'result-panel', 'history-panel'].map((regionId) => ({ regionId, kind: 'section', label: regionId })), controls, resultSurfaces: [{ surfaceId: 'result-panel', pageId: 'submission', kind: 'container' }, { surfaceId: 'history-panel', pageId: 'submission', kind: 'container' }], states: ['empty', 'processing', 'success', 'failure'] }];
  inventory.design.manifestDigest = designManifestDigest; inventory.authoringStatus = 'completed-by-fixture-agent';
  writeJSON(`${dir}/frontend-semantic-inventory.json`, inventory);
  writeJSON(`${dir}/observed-interactions.json`, { schemaVersion: '1.0', sourceType: 'finalized-design', designManifestDigest, interactions: [] });
  writeJSON(`${dir}/asset-role-inventory.json`, { schemaVersion: '1.0', sourceType: 'finalized-design', designManifestDigest, assets: [] });
  const map = readJSON(`${dir}/control-capability-map.json`); delete map.releaseDigest; map.designManifestDigest = designManifestDigest; for (const item of map.mappings) { item.mappingType = 'designed-trigger'; item.evidence = { status: 'designed', sources: ['design:submission'] }; } writeJSON(`${dir}/control-capability-map.json`, map);
  const ledger = readJSON(`${dir}/control-dispositions.json`); delete ledger.releaseDigest; ledger.designManifestDigest = designManifestDigest; for (const item of ledger.dispositions) item.evidence = { status: 'designed', sources: ['design:submission'] }; writeJSON(`${dir}/control-dispositions.json`, ledger);
  for (const file of ['functional-spec.json', 'capability-definitions.json']) { const value = replaceFixtureSources(readJSON(`${dir}/${file}`)); value.visualSource = { sourceType: 'finalized-design', designManifestDigest, pageIds: ['submission'], routes: { submission: '/submission' } }; for (const capability of value.capabilities || []) for (const operation of capability.operations || []) { delete operation.providerContract; delete operation.integrationVerification; delete operation.integrationBindings; } writeJSON(`${dir}/${file}`, value); }
}
function removePlannedFixtureCapability(dir) {
  const spec = readJSON(`${dir}/functional-spec.json`); removedDesignCapability = spec.capabilities.find((item) => item.specificationStatus === 'planned'); const ids = new Set(spec.capabilities.filter((item) => item !== removedDesignCapability).map((item) => item.id)); spec.capabilities = spec.capabilities.filter((item) => ids.has(item.id)); spec.journeys = (spec.journeys || []).map((journey) => ({ ...journey, capabilityIds: journey.capabilityIds.filter((id) => ids.has(id)), operationIds: journey.operationIds.filter((id) => spec.capabilities.some((capability) => capability.operations.some((operation) => operation.id === id))), steps: journey.steps.filter((step) => ids.has(step.capabilityId)) }));
  writeJSON(`${dir}/functional-spec.json`, spec); const definitions = readJSON(`${dir}/capability-definitions.json`); definitions.capabilities = spec.capabilities; definitions.journeys = spec.journeys; writeJSON(`${dir}/capability-definitions.json`, definitions);
  const pageMap = readJSON(`${dir}/page-function-map.json`); for (const page of pageMap.pages) page.capabilityIds = page.capabilityIds.filter((id) => ids.has(id)); writeJSON(`${dir}/page-function-map.json`, pageMap);
  const controlMap = readJSON(`${dir}/control-capability-map.json`); const removedControlIds = new Set(controlMap.mappings.filter((item) => item.capabilityId === removedDesignCapability.id).map((item) => item.controlId)); controlMap.mappings = controlMap.mappings.filter((item) => ids.has(item.capabilityId)); writeJSON(`${dir}/control-capability-map.json`, controlMap);
  const inventory = readJSON(`${dir}/frontend-semantic-inventory.json`); for (const page of inventory.pages) page.controls = page.controls.filter((item) => !removedControlIds.has(item.controlId)); writeJSON(`${dir}/frontend-semantic-inventory.json`, inventory);
  const ledger = readJSON(`${dir}/control-dispositions.json`); ledger.dispositions = ledger.dispositions.filter((item) => !removedControlIds.has(item.controlId)); writeJSON(`${dir}/control-dispositions.json`, ledger);
  const anchored = new Set(); for (const capability of spec.capabilities) for (const question of ['userInput', 'systemBehavior', 'output', 'resultDestination', 'failures', 'downstreamUse']) for (const anchor of capability.closure?.[question]?.evidenceAnchors || []) anchored.add(anchor);
  const index = readJSON(`${dir}/evidence-index.json`); const dispositions = readJSON(`${dir}/evidence-dispositions.json`); const dispositioned = new Set(dispositions.dispositions.map((item) => item.evidenceId)); for (const item of index.evidence) if (!anchored.has(item.id) && !dispositioned.has(item.id)) dispositions.dispositions.push({ evidenceId: item.id, reason: 'out-of-scope', rationale: 'Evidence belongs only to the optional branch excluded from this complete design-led vertical fixture.' }); writeJSON(`${dir}/evidence-dispositions.json`, dispositions);
  const manifest = readJSON(`${dir}/manifest.json`); manifest.deliveryMode = 'complete'; manifest.productCompletionClaim = 'complete'; manifest.capabilitySummary = { ...manifest.capabilitySummary, total: spec.capabilities.length, complete: spec.capabilities.length, planned: 0 }; writeJSON(`${dir}/manifest.json`, manifest);
}
function replaceFixtureSources(value) { if (Array.isArray(value)) return value.map(replaceFixtureSources); if (value && typeof value === 'object') return Object.fromEntries(Object.entries(value).map(([key, item]) => [key, replaceFixtureSources(item)])); return typeof value === 'string' && value.startsWith('visual-release:') ? 'design:submission' : value; }
