...
const uploadChallenges = Object.fromEntries([...operations.values()].filter((operation) => operation.resourceTransfer).map((operation) => {
  const range = resourceFileRange(operation); const count = range.max >= 2 ? Math.max(2, range.min) : Math.max(1, range.min);
  const files = Array.from({ length: count }, (_, index) => { const bytes = randomBytes(97 + index); const path = `${cleanDataDir}/upload-${sha(Buffer.from(`${operation.id}:${index}`)).slice(0, 12)}.bin`; writeFileSync(path, bytes); return { path, sha256: sha(bytes), bytes: bytes.length }; });
  const paths = files.map((item) => item.path);
  return [operation.id, { ...files[0], path: paths.length === 1 ? paths[0] : paths, files, paths, minFiles: range.min, maxFiles: range.max, fileField: operation.resourceTransfer.fileField }];
}));
...
function validatePlaywrightTrace(raw, baseUrl, originReceipts) {
  ...
  for (const { item, event, pageUrl } of actions) {
    ...
    const selectedPaths = Array.isArray(selectedFiles) ? selectedFiles : selectedFiles ? [selectedFiles] : []; const challenge = uploadChallenges[operationId]; const challengeFiles = challenge?.files || (challenge ? [challenge] : []); const challengePaths = challengeFiles.map((item) => item.path); const multipartFiles = operationRequest?.requestBody?.files?.[operation?.resourceTransfer?.fileField] || [];
    ...
    const fileSelectionEvidence = operation?.resourceTransfer ? { operationId, controlId: fileBinding?.controlId, fileField: operation.resourceTransfer.fileField, actionCallId: fileSelectionAction?.callId, observedAction: fileSelectionAction?.method, selectedCount: selectedPaths.length, selectedChallenge: JSON.stringify([...selectedPaths].sort()) === JSON.stringify([...challengePaths].sort()), requestMatched: Boolean(operationRequest), requestContentType: operationRequest?.contentType, contractContentType: operation.resourceTransfer.contentType || operation.request?.contentType, multipartFiles, responseIntegrityDigests, challengeDigestMatched: (multipartFiles.length === challengeFiles.length && challengeFiles.every((item) => multipartFiles.some((file) => file.sha256 === item.sha256 && file.bytes === item.bytes))) || challengeFiles.every((item) => responseIntegrityDigests.includes(item.sha256)) } : undefined;
