...
During each observed ingress request the application makes its real outbound calls to the external observer. The campaign injects a private random token per provider operation into the application process; the adapter forwards the matching token as `x-validation-operation-token`. E2E does not receive these tokens. External calls are attributed by token rather than a shared challenge/time window, so one invocation cannot prove two operations. Every invocation is checked against every required `integrationBinding`; one complete first call cannot cover incomplete later calls. Ingress values are recorded at `request.*`, `request.query.*`, `request.path.*`, and `request.header.*`; multipart fields and files are recorded by field name with safe content metadata/digests. Identity bindings require equal source and provider digests. For a locked FDD `resourceResolution`, the observer requires the upload response's dynamic resource ID in the downstream request and the uploaded content digest in the provider target; two unrelated values cannot prove resolution. For `independent-items`, the collection is read from the FDD result contract's exact `responsePath`, never from the longest convenient array. The observer enforces distinct-call/result/cardinality and the declared concurrency ceiling. Provider-to-business transformation is described by the Agent-authored result lineage and judged through the bound result review; the runner does not require a raw provider ID to leak into the application response. `operation-observation-receipt.json` schema `1.5` contains one receipt per provider operation, and all findings must be empty.

Domain-specific quality judgment is performed by a distinct reviewer Agent against the campaign-owned result artifacts and the approved acceptance criteria. When FDD declares `integrationVerification.resultReview.required`, the campaign writes `result-review-request.json` after stopping the candidate E2E and application, then waits for the Agent-authored `result-review-receipt.json`. The request binds the implementation Agent identities from BMAD completion, the actual provider responses, the application result, and `providerResultLineage`; the receipt reviewer must differ from every bound implementation Agent and cover the exact assertion set and request digest. Scripts check only this input/output binding; they do not decide semantic quality.

The external observer does not invent a provider response protocol. For each operation it replays the locked FDD `providerContract.controlledResponse`, injects the invocation's unique value at `resultIdPath`, and returns the declared status and content type. The FDD Agent is responsible for authoring that fixture from protocol evidence, the PI Agent is responsible for using the production adapter parsing path, and the independent reviewer checks semantic fidelity.

The campaign does not require the provider's raw result ID to appear in the application response. FDD declares the provider-to-business transformation in `providerResultLineage`; the review request carries both sides, and the reviewer Agent judges the declared lineage. This permits persistence, resource download, business-owned IDs, and other Agent-implemented transformations without teaching the runner their semantics.
