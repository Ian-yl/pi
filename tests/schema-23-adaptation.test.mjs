...
test('the golden simulated flow is a schema 2.3 functional package that verifies end to end', () => {
  assert.equal(readJSON(path.join(golden, 'functional-domain/manifest.json')).schemaVersion, '2.3');
  const result = spawnSync('node', [verify, path.join(golden, 'implementation'), '--require-level', 'simulated'], { encoding: 'utf8' });
  assert.equal(result.status, 0, result.stderr);
});

// A schema 2.3 package that declares the semantic-file contract but is missing one of those files
// must fail closed.
test('schema 2.3 prepare rejects a functional package missing a declared semantic file (no fail-open)', () => {
  ...
  try {
    ...
    assert.equal(readJSON(path.join(dir, 'fn/manifest.json')).schemaVersion, '2.3');
    ...
    assert.notEqual(result.status, 0, 'a 2.3 package missing a declared semantic file must be rejected');
    assert.match(`${result.stdout}${result.stderr}`, /asset-role-inventory\.json/);
  } finally { rmSync(dir, { recursive: true, force: true }); }
});

test('schema 2.3 verify rejects a prepared workspace whose declared semantic input was removed (no fail-open)', () => {
  ...
  try {
    ...
    assert.notEqual(result.status, 0, 'a 2.3 workspace missing a declared semantic input must be rejected');
  } finally { rmSync(dir, { recursive: true, force: true }); }
});

test('schema 2.3 prepare rejects a functional package missing evidence dispositions', () => {
...
test('schema 2.3 prepare rejects a locked design file that is absent', () => {
...
test('schema 2.3 verify rejects removal of a locked evidence input', () => {
...
test('schema 2.3 verify rejects removal of a copied locked design', () => {
