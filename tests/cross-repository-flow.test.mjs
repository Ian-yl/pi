...
test('FDD schema 2.3 authoring flows through reviewed handoff to verified PI implementation', () => {
  ...
  try {
    ...
    assert.equal(functionalManifest.schemaVersion, '2.3');
    assert.equal(functionalManifest.status, 'approved');
    assert.equal(planningManifest.status, 'approved');
    assert.equal(functionalReceipt.contractVersion, 'functional-domain/2.3');
    ...
    assert.equal(handoffReceipt.contractVersion, 'implementation-handoff/2.3');
